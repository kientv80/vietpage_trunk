...
public class CompanyBean {
	...
	public CompanyInfo getCompanyInfo(long id) throws Exception {
		JCSCacheClient cache = JSCCacheManager.getInstace().getPageCache();
		Object tmp = cache.get(id);
		if (tmp != null)
			return (CompanyInfo) tmp;
		else
			System.out.println(">>>>>>>>>>>>>>>> page is not cache");
	...
	private void clearCompanyCache(long comId) {
		try {
			JSCCacheManager.getInstace().getPageCache().remove(comId);
	...
	public int addCompanyInfo(CompanyInfo c) throws Exception {
		...
		try {
			con = JDBCConnection.getInstance().getConnection();
			stm = con.prepareStatement("insert into company(name,url,headerimage,description,contentimage,location_map_image,location_id,service_id,owner_id,phones, emails,addresses,providedservice,pagename,news) values(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)");
			stm.setString(1, c.getName());
			stm.setString(2, c.getUrl());
			stm.setString(3, c.getHeaderImage());
			stm.setString(4, c.getDescription());
	...
	public List<CompanyInfo> getCompanyInfo(int cityId, int locationId, int serviceTypeId, String searchText) throws Exception {
		...
		if (searchText == null) {
			JCSCacheClient cache = JSCCacheManager.getInstace().getPageLocationService();
			Object coms = cache.get(cityId + "_" + locationId + "_" + serviceTypeId);
			if (coms != null && coms instanceof List)
				companies = (List<CompanyInfo>) coms;
			if(companies.size() > 0)
				return companies;
			else
				System.out.println(">>>>>>>>>>>>>>>> cache com is empty");
		}
		Connection con = null;
		PreparedStatement stm = null;
		ResultSet result = null;
		try {
			...
			if (searchText == null) {
				JCSCacheClient cache = JSCCacheManager.getInstace().getPageLocationService();
				cache.put(cityId + "_" + locationId + "_" + serviceTypeId, companies);
			}
		} catch (Exception e) {
			log.error("", e);
		} finally {
	...
	private List<CompanyInfo> toCompanyInfo(ResultSet result) throws SQLException {
		...
		while (result.next()) {
			...
			company.setNews(result.getString("news"));
