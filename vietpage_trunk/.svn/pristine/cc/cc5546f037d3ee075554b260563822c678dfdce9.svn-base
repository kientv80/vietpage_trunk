		...
		try {
			if ("updateRegisterInfo".equals(getAction()) && "post".equalsIgnoreCase(getHttpServletRequest().getMethod())) {
				...
				if(file != null){
					...
					Map<String,String> image = uploadFile("map_" + Calendar.getInstance().getTimeInMillis() + ".jpg", comInfo.getId()+"",UPLOAD_OBJECT.OTHERS);
					update.setMapImage(image.get(StandarLayout.IMAGE));
				}
				if(getLocationId() > 0)
				...
				System.out.println("serviceTypeId=" + serviceTypeId+ "getLocationId="+ getLocationId()+"comInfo.getId()=" + comInfo.getId());
				if(serviceTypeId > 0)
					update.setServiceTypeId(serviceTypeId);
				else
					update.setServiceTypeId(comInfo.getServiceTypeId());
