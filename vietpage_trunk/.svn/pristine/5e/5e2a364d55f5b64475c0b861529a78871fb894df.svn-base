	...
	public String execute() {
		...
		try {
			...
			if("search".equals(getAction())){
				setCompanies(CompanyBean.getInstance().getCompanyInfo(selectedCity, locationId, serviceTypeId, searchText));
				...
				items = MenuBean.getInstance().searchItems(100, selectedCity, getLocationId(),getServiceTypeId(),searchText);
				if(searchText != null && !searchText.isEmpty()){
					for(Item i : items){
						i.setName(i.getName().replaceAll(searchText, "<b>"+searchText+"</b>"));
					}
				}
