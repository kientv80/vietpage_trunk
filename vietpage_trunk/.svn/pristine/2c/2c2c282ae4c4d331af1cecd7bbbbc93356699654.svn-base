...
<c:if test="${feeds != null && feeds.size() > 0 }">
	<s:iterator value="feeds" var="f">
		<div style="float: left;width: 100%; text-align: left;vertical-align: left;padding: 0px 20px 10px 20px;;background-color: #dadce0;">
		<div class="itemContent">
			<c:if test="${f.image !=null && !f.image.isEmpty()}">
					<a href="/items?comid=${f.comid }">
						...
						<img  title="${f.title }" alt="${f.title }" src="${f.image }" style="width: 100%;">
						 <br><font size="1.5">
